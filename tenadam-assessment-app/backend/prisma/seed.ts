...
async function main() {
  ...
  for (const category of allCategories) {
    const categoryQuestions = allQuestions[category.name as keyof typeof allQuestions]
    if (categoryQuestions) {
      for (const subcategory of category.subcategories) {
        const subcatQuestions = categoryQuestions[subcategory.name as keyof typeof categoryQuestions]
        if (subcatQuestions) {
